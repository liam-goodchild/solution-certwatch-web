...
async function handler(
  ...
  try {
    ...
    if (
      !VALID_VENDORS.includes(body.vendor as (typeof VALID_VENDORS)[number])
    ) {
      return {
        status: 400,
        jsonBody: {
          error: `vendor must be one of: ${VALID_VENDORS.join(", ")}.`,
        },
      };
    }
    if (
      ...
      return {
        status: 400,
        jsonBody: { error: "expirationDate must be in YYYY-MM-DD format." },
      };
    }
    ...
    const { resource } = await containers
      .certifications()
      .items.create<Certification>(cert);
